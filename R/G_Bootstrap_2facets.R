calcVarComp <- function(x){
  ##Calculate 2-facet variance components via ANOVA from "long" data
  #Get sample sizes
  np <- nlevels(x$p)
  ni <- nlevels(x$i)
  no <- nlevels(x$o)
  ...
  SSio <- Tio - Ti - To + Tmu
  SStot <- Tpio - Tmu
  SSpio <- SStot - SSp - SSi - SSo - SSpi - SSpo - SSio
...
calcAdjustedVar <- function(.data, .var_p, .var_i, .var_o, .var_pi, .var_po, .var_io, .var_pio, type){
  ...
  if (type == "pi"){
    ...
    adj_var_o <- .var_o - (1/(np - 1))*.var_po - (1/(ni - 1))*.var_io + (1/((np - 1)*(ni - 1)))*.var_pio
  ...
  if (type == "po"){
    ...
    adj_var_i <- .var_i - (1/(np - 1))*.var_pi - (1/(no - 1))*.var_io + (1/((np - 1)*(no - 1)))*.var_pio
  ...
  if (type == "io"){
    adj_var_p <- .var_p - (1/(ni - 1))*.var_pi - (1/(no - 1))*.var_po + (1/((ni - 1)*(no - 1)))*.var_pio
  ...
  if (type == "pio"){
    adj_var_p <- (np/(np - 1))*.var_p - (np/((np - 1)*(ni - 1)))*.var_pi - (np/((np - 1)*(no - 1)))*.var_po + (np/((np - 1)*(ni - 1)*(no - 1)))*.var_pio
    adj_var_i <- (ni/(ni - 1))*.var_i - (ni/((np - 1)*(ni - 1)))*.var_pi - (ni/((ni - 1)*(no - 1)))*.var_io + (ni/((np - 1)*(ni - 1)*(no - 1)))*.var_pio
    adj_var_o <- (no/(no - 1))*.var_o - (no/((np - 1)*(no - 1)))*.var_po - (no/((ni - 1)*(no - 1)))*.var_io + (no/((np - 1)*(ni - 1)*(no - 1)))*.var_pio
    adj_var_pi <- (np*ni/((np - 1)*(ni - 1)))*.var_pi - (np*ni/((np - 1)*(ni - 1)*(no - 1)))*.var_pio
    adj_var_po <- (np*no/((np - 1)*(no - 1)))*.var_po - (np*no/((np - 1)*(ni - 1)*(no - 1)))*.var_pio
    adj_var_io <- (ni*no/((ni - 1)*(no - 1)))*.var_io - (ni*no/((np - 1)*(ni - 1)*(no - 1)))*.var_pio
...
summaryCI <- function(.result, ConfLevel, rounding){
  ...
  colnames(gstudy_est) <- c("SubjVar", "Facet1Var", "Facet2Var", "Subj x F1 Var", "Subj x F2 Var", "F1 x F2 Var","ResidVar", "SubjVar_SE", "Facet1Var_SE", "Facet2Var_SE", "Subj x F1 Var_SE", "Subj x F2 Var_SE", "F1 x F2 Var_SE","ResidVar_SE")
  #rownames(gstudy_est) <- c("boot-p")
  #CIs
  ...
  colnames(gstudy_cis) <- c("SubjVar", "Facet1Var", "Facet2Var", "Subj x F1 Var", "Subj x F2 Var", "F1 x F2 Var","ResidVar")
  #rownames(gstudy_cis) <- c("boot-p")
  #DstudyEstimates (mean and standard error)
  dstudy_est <- round(rbind(c(means[8:10], sds[8:10])), 4)
  colnames(dstudy_est) <- c("AbsErrVar", "GenCoef", "DepCoef", "AbsErrVar_SE", "GenCoef_SE", "DepCoef_SE")
  #rownames(dstudy_est) <- c("boot-p")
  #DstudyCIs
